...
def decrypt_file_in_chunks(encrypted_text, key):
    chunk_size = 1024
    iv = b'\x00' * 16  # ⚠️ Replace with the same IV used during encryption
...
class SelfEncryptionApp:
    def __init__(self, master):
        ...
        # Load images
        self.logo = PhotoImage(file=r"C:\Users\gudal\OneDrive\Desktop\CNS\download.png")
    ...
    def decrypt(self):
        ...
        if file_path and aes_key and encrypted_text:
            ...
            if authenticate(username, password):
                ...
                decrypted_text_bytes = decrypt_file_in_chunks(encrypted_text, aes_key)
                try:
                    # Convert bytes to readable English text
                    decrypted_text = decrypted_text_bytes.decode("utf-8")
                except UnicodeDecodeError:
                    # If it’s not a text file, fallback to hex
                    decrypted_text = decrypted_text_bytes.hex()

                print("Decrypted Content (English):", decrypted_text)
                # Compute SHA hash of the decrypted text
                sha_hash_decrypted = hashlib.sha256(decrypted_text.encode("utf-8")).hexdigest()

                # Show English text
                print("Decrypted Content (English):", decrypted_text)

                # Optional: also show hex if you want both
                decrypted_hex = decrypted_text_bytes.hex()
